...
function buildWorkletString(t, fun, closureVariables, name, inputMap) {
  function prependClosureVariablesIfNecessary() {
    const closureDeclaration = t.variableDeclaration("const", [t.variableDeclarator(t.objectPattern(closureVariables
    ...
    .filter(v => v.name !== "Worklets").map(variable => t.objectProperty(t.identifier(variable.name), t.identifier(variable.name), false, true))), t.identifier("jsThis"))]);
...
function makeWorklet(t, fun, state) {
  ...
  const transformed = transformSync(code, {
    ...
    plugins: ["@babel/plugin-transform-shorthand-properties", "@babel/plugin-transform-arrow-functions", ["@babel/plugin-proposal-optional-chaining", {
      loose: true
  ...
  traverse(transformed.ast, {
    ReferencedIdentifier(path) {
      ...
      if (global.hasOwnProperty(name) || globals.has(name) || fun.node.id && fun.node.id.name === name) {
        return;
      }
  ...
  const [funString] = buildWorkletString(t, transformed.ast, variables, functionName, transformed.map);
  ...
  const statements = [t.variableDeclaration("const", [t.variableDeclarator(privateFunctionId, funExpression)]), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("_closure"), false), t.objectExpression(variables.map(variable => t.objectProperty(t.identifier(variable.name), variable, false, true))))), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("asString"), false), t.stringLiteral(funString))), t.expressionStatement(t.assignmentExpression("=", t.memberExpression(privateFunctionId, t.identifier("__location"), false), t.stringLiteral(location)))];
...
function processWorklets(t, path, state) {
  const name = path.node.callee.type === "MemberExpression" ? path.node.callee.property.name : path.node.callee.name;
